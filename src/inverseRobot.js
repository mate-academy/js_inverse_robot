...
`
Create a 'inverseRobot' function that takes 'robot' as a parameter
and returns a new object in which keys will change places with values.
`
...
function inverseRobot(robot) {
  ...
  for (const key in robot) {
    if (robot.hasOwnProperty(key)) {
      ...
      // we are getting the actual key name
      // and we store it in variable, named 'value'
